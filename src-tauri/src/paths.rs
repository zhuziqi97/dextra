//! Centralized resolution of dextra-owned filesystem paths.
//!
//! Mirrors the conventions already used by `preferences.rs` (`~/.dextra/`)
//! and `experts.rs` (`~/.dextra/skills/`). New features that need a
//! user-scoped persistent directory should call into this module instead of
//! re-deriving `dirs::home_dir().join(".dextra")` themselves.
...
/// `$DEXTRA_HOME` if set (and non-empty), else `~/.dextra/`.
///
/// Returns the relative `.dextra` path when no home directory is available;
/// callers must still handle creation failures gracefully.
pub fn dextra_home_dir() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
        return PathBuf::from(custom);
    }
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME))
}

/// Root directory for desktop-pet assets.
///
/// Resolution order:
/// 1. `$DEXTRA_HOME/pets` (explicit override, used in tests and custom installs)
/// 2. `$DEXTRA_DATA_DIR/pets` (server-mode data directory, populated by
///    `dextra-server` from the corresponding env var)
/// 3. `~/.dextra/pets` (default for the desktop app)
pub fn dextra_pets_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(PETS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(PETS_DIR_NAME))
}
...
///
/// Resolution order matches `dextra_pets_root()`.
pub fn dextra_browser_profiles_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(BROWSER_PROFILES_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(BROWSER_PROFILES_DIR_NAME))
}

/// Root directory for attachments uploaded from the web client.
///
/// Resolution order matches `dextra_pets_root()`:
/// 1. `$DEXTRA_HOME/uploads`
/// 2. `$DEXTRA_DATA_DIR/uploads` (server-mode data directory)
/// 3. `~/.dextra/uploads` (desktop default)
///
/// Files in this directory are not garbage-collected by dextra itself —
...
/// `DEXTRA_UPLOAD_MAX_TOTAL_BYTES` (see `web::handlers::files`): new
...
/// in-flight reservation counter to make `DEXTRA_UPLOAD_MAX_TOTAL_BYTES`
/// a hard ceiling within one `dextra-server` process. Multiple
/// `dextra-server` processes sharing the same uploads root (e.g.
...
pub fn dextra_uploads_root() -> PathBuf {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(UPLOADS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(UPLOADS_DIR_NAME))
}

/// Root directory for the user-selected workspace background image.
///
/// Resolution mirrors [`dextra_pets_root`] exactly:
/// 1. `$DEXTRA_HOME/backgrounds` (explicit override)
/// 2. `$DEXTRA_DATA_DIR/backgrounds` (server-mode data directory)
/// 3. `~/.dextra/backgrounds` (desktop default)
pub fn dextra_backgrounds_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(BACKGROUNDS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(BACKGROUNDS_DIR_NAME))
}

/// Root directory for application diagnostic logs (rotating files written by
/// the `tracing` file appender; see `crate::logging`).
///
/// Resolution mirrors [`dextra_uploads_root`] exactly so logs land on the same
/// filesystem root as uploads/pets/the database:
/// 1. `$DEXTRA_HOME/logs` (explicit override)
/// 2. `$DEXTRA_DATA_DIR/logs` (server-mode data directory)
/// 3. `~/.dextra/logs` (default for the desktop app)
///
/// Pure env + `dirs::home_dir()`, so it is callable at the very start of a
/// process — before the database (or, in `dextra-server`, the tokio runtime)
/// exists — which is exactly when the subscriber must be installed.
pub fn dextra_logs_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(LOGS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(LOGS_DIR_NAME))
}

/// Root directory for dextra's own per-turn timing observations (see
...
/// history parser.
///
/// Resolution mirrors [`dextra_uploads_root`]:
/// 1. `$DEXTRA_HOME/turn-timings`
/// 2. `$DEXTRA_DATA_DIR/turn-timings` (server-mode data directory)
/// 3. `~/.dextra/turn-timings` (desktop default)
pub fn dextra_turn_timings_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(TURN_TIMINGS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(TURN_TIMINGS_DIR_NAME))
}

/// Root directory for dextra's own ACP transcripts (see
/// `crate::acp_transcript`) — the raw `session/update` stream and outgoing
/// prompts recorded for **custom ACP agents**, which have no dextra-side
/// transcript parser of their own. Written by the live connection, read back
/// by `crate::parsers::acp_native`.
///
/// Resolution mirrors [`dextra_turn_timings_root`]:
/// 1. `$DEXTRA_HOME/acp-transcripts`
/// 2. `$DEXTRA_DATA_DIR/acp-transcripts` (server-mode data directory)
/// 3. `~/.dextra/acp-transcripts` (desktop default)
pub fn dextra_acp_transcripts_root() -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_HOME").filter(|s| !s.is_empty()) {
    ...
    dirs::home_dir()
        .map(|h| h.join(DEXTRA_DIR_NAME).join(ACP_TRANSCRIPTS_DIR_NAME))
        .unwrap_or_else(|| PathBuf::from(DEXTRA_DIR_NAME).join(ACP_TRANSCRIPTS_DIR_NAME))
}
...
///
/// Resolution:
/// 1. If `DEXTRA_DATA_DIR` is set and non-empty, return its absolutized
...
/// authoritative and not re-read `DEXTRA_DATA_DIR` themselves; the
/// startup code in `lib.rs` / `bin/dextra_server.rs` writes the
...
/// consult `DEXTRA_DATA_DIR` — it returns the identifier-derived path
...
/// operator pre-set `DEXTRA_DATA_DIR` to a custom location.
pub fn resolve_effective_data_dir(tauri_fallback: &Path) -> PathBuf {
    if let Some(custom) = std::env::var_os("DEXTRA_DATA_DIR").filter(|s| !s.is_empty()) {
...
// The resolver functions above depend on global env vars (`DEXTRA_HOME`,
// `DEXTRA_DATA_DIR`), so unit tests would need cross-test serialization to
// avoid races. The behaviour is covered end-to-end by `pets::*` tests which
// set `DEXTRA_HOME` inside a serialized test mutex; we deliberately don't
...
mod tests {
    ...
    fn simplifies_a_verbatim_drive_path() {
        assert_eq!(
            simplify_verbatim_path(Path::new(r"\\?\C:\Users\song\.dextra\uploads\b\img.png")),
            PathBuf::from(r"C:\Users\song\.dextra\uploads\b\img.png")
        );
    }
    ...
    fn leaves_every_other_shape_untouched() {
        for path in [
            "/home/u/.dextra/uploads/b/img.png",
