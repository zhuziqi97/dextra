...
//!
//! Experts are curated skills (from obra/superpowers) that dextra bundles
//! into its binary via `include_dir!`. On startup they are extracted to a
//! central directory `~/.dextra/skills/<id>/`. Users can then enable an
...
//! database state, and updates propagate automatically when dextra upgrades
...
const CENTRAL_DIR_NAME: &str = ".dextra";
...
pub(crate) fn classify_link(link_path: &Path, expected_target: &Path) -> ExpertLinkState {
    ...
    match (resolved_link, resolved_expected) {
        ...
        (Some(l), Some(e)) if paths_equivalent(&l, &e) => ExpertLinkState::LinkedToDextra,
...
mod tests {
    ...
    async fn apply_links_does_not_deadlock() {
        ...
        let ops = vec![
            LinkOp {
                expert_id: "zzz-dextra-batch-test-absent-aaa".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
                expert_id: "zzz-dextra-batch-test-absent-bbb".into(),
    ...
    async fn apply_links_collects_per_op_results_without_aborting() {
        let ops = vec![
            LinkOp {
                expert_id: "zzz-dextra-batch-test-absent".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
