...
const GOOGLE_URI = "dextra://embedded/https%3A%2F%2Fwww.google.com%2F"
/** …and the one the chip for the site it came from is keyed by. */
const GOOGLE_SITE = "dextra://embedded/https%3A%2F%2Fwww.google.com"
/** The same pair for linux.do. */
const LINUX_DO_URI = "dextra://embedded/https%3A%2F%2Flinux.do%2F"
const LINUX_DO_SITE = "dextra://embedded/https%3A%2F%2Flinux.do"
...
describe("extractUserResourcesFromText — dextra references stay inline", () => {
  ...
  it("lists two pages of one site under it once", () => {
    const topic = "dextra://embedded/https%3A%2F%2Flinux.do%2Ft%2Ftopic%2F1"
    const latest =
      "dextra://embedded/https%3A%2F%2Flinux.do%2Flatest%3Fpage%3D2"
    const { resources } = extractUserResourcesFromText(
  ...
  it("does not read the page's own markup as the sender's references", () => {
    ...
    expect(resources).toEqual([
      {
        name: "shop.test",
        uri: "dextra://embedded/https%3A%2F%2Fshop.test",
        mime_type: null,
      },
    ])
    expect(text).toBe("hi")
...
describe("adaptMessageTurn — user reference resources", () => {
  ...
  it("names a screenshot the way the composer did, and lists its page", () => {
    ...
    const uri = "dextra://embedded/https%3A%2F%2Flinux.do%2Ft%2Ftopic%2F1"
  ...
  it("names a page's console lines by how many there were", () => {
    ...
    const uri = "dextra://embedded/http%3A%2F%2F127.0.0.1%3A3000%2F"
    expect(adapted.content).toEqual([
    ...
    expect(adapted.userResources).toEqual([
      {
        name: "127.0.0.1:3000",
        uri: "dextra://embedded/http%3A%2F%2F127.0.0.1%3A3000",
        mime_type: null,
      },
    ])
  })
  ...
  // What Dextra's own projection sends a viewer on another client: the block's
  ...
  it("reads a projected page whose title wrote the block's own tag", () => {
    ...
    expect(adapted.content).toEqual([
      {
        type: "text",
        text: "[Page screenshot](dextra://embedded/http%3A%2F%2Fx.test%2F)",
      },
    ])
    expect(adapted.userResources).toEqual([
      {
        name: "x.test",
        uri: "dextra://embedded/http%3A%2F%2Fx.test",
        mime_type: null,
      },
    ])
  })
  ...
  it("keeps an address the person typed that matches a block's", () => {
    ...
    expect(adapted.content).toEqual([
      {
        type: "text",
        text: "[a.title.raw-link.raw-topic-link](dextra://embedded/https%3A%2F%2Flinux.do%2F) https://linux.do/",
      },
    ])
  })
})
